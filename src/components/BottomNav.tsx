...
export default function BottomNav() {
  ...
  return (
    <nav className="bottom-nav">
      <div className="max-w-md mx-auto flex justify-around items-end h-16">
        <Link href="/" className={`nav-item ${pathname === '/' ? 'nav-item-active' : 'nav-item-inactive'}`}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
          <span style={{ fontSize: 10, fontWeight: 600 }}>홈</span>
        </Link>

        <Link href="/new" className="nav-center-btn">
        ...
        <Link href="/profile" className={`nav-item ${pathname === '/profile' ? 'nav-item-active' : 'nav-item-inactive'}`}>
          ...
          <span style={{ fontSize: 10, fontWeight: 600 }}>프로필</span>
        </Link>
      </div>
    </nav>
  );
